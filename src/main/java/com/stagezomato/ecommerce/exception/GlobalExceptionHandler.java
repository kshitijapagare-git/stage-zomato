...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(OrderItemConflictException.class)
    public ResponseEntity<Map<String, Object>> handleOrderItemConflict(OrderItemConflictException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(body(HttpStatus.CONFLICT, ex.getMessage()));
    }

    @ExceptionHandler(CartItemConflictException.class)
    public ResponseEntity<Map<String, Object>> handleCartItemConflict(CartItemConflictException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(body(HttpStatus.CONFLICT, ex.getMessage()));
    }

    @ExceptionHandler(ProductConflictException.class)
    public ResponseEntity<Map<String, Object>> handleProductConflict(ProductConflictException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(body(HttpStatus.CONFLICT, ex.getMessage()));
    }
